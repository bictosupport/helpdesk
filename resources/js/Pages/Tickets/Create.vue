            ...
            <form @submit.prevent="store">
                <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
                    ...
                    <select-input 
                        ...
                        :label="__('Department')" 
                        :required="true"
                        :disabled="auth.user.role.slug !== 'admin'"> 
                        <option :value="null" />
                        <option v-for="s in departments" :key="s.id" :value="s.id">{{ s.name }}</option>
                    </select-input>
...
export default {
    ...
    data() {
        return {
            ...
            form: this.$inertia.form({
                ...
                priority_id: null,
                status_id: null,
                department_id: this.$page.props.user_department_id,
                category_id: null,
                assigned_to: null,
                type_id: null,
                subject: null,
    ...
    created() {
        ...
        this.setDefaultValue(this.departments, 'department_id', this.$page.props.user_department_id); // Set default department ID based on the user's department
    },
    methods: {
        uploader(editor) {
        ...
        setDefaultValue(arr, key, value) {
            const find = arr.find(i => i.id === value); // Match by ID instead of name
            if (find) {
                this.form[key] = find.id; // Set the department_id from the selected department
            }
        },
        fileInputChange(e) {
